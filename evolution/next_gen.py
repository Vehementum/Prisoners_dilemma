...
def create_strategies_pop(strategies, pop_size):
    """Creates a list of tuples (name, strategy_function, population) from strategies dict and population size."""
    strategies_pop = []
    pop = pop_size // len(strategies)
    for name, func in strategies.items():
        strategies_pop.append((name, func, pop))
    return strategies_pop
...
def next_gen(strategies_pop, interval=5):
    ...
    for i in range(len(strategies_pop)):
        match_hist = match_history(strategies_pop[i][1], strategies_pop[i][1], interval)
    ...
    for i in range(len(strategies_pop)-1):
        for j in range(i+1, len(strategies_pop)):
            match_hist = match_history(strategies_pop[i][1], strategies_pop[j][1], interval)
            score1, score2 = score(match_hist)
...
def next_gen_with_noise(strategies_pop, interval=5):
    ...
    total_score = 0
    for i in range(len(strategies_pop)):
        match_hist = match_history_with_noise(strategies_pop[i][1], strategies_pop[i][1], interval)
        score1, score2 = score(match_hist)
        total_score += score1*score_coeff(strategies_pop[i][2])
        next_generation[i] = (next_generation[i][0], next_generation[i][1], next_generation[i][2] + score1 * score_coeff(strategies_pop[i][2]))
    for i in range(len(strategies_pop)-1):
        for j in range(i+1, len(strategies_pop)):
            match_hist = match_history_with_noise(strategies_pop[i][1], strategies_pop[j][1], interval)
            score1, score2 = score(match_hist)
            total_score += (score1 + score2) * strategies_pop[i][2] * strategies_pop[j][2]
            next_generation[i] = (next_generation[i][0], next_generation[i][1], next_generation[i][2] + score1 * strategies_pop[i][2] * strategies_pop[j][2])
            next_generation[j] = (next_generation[j][0], next_generation[j][1], next_generation[j][2] + score2 * strategies_pop[i][2] * strategies_pop[j][2])
...
def evolution_data(strategies, pop_size=100, generations=50, interval=5):
    ...
    for gen in range(generations):
        ...
        strategies_pop = next_gen(strategies_pop, interval)
    return history


def evolution_data_with_noise(strategies, pop_size=100, generations=50, interval=5):
    """Simulates evolution of strategies over a number of generations.
    strategies: dict of strategy_name: strategy_function
    pop_size: total population size
    generations: number of generations to simulate
    ...
    for gen in range(generations):
        history.append([(s[0], s[2]) for s in strategies_pop])  # Store name and population
        strategies_pop = next_gen_with_noise(strategies_pop, interval)
    return history